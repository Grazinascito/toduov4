...
import { Send, Plus, Trash2, UserPlus, Play, Pause, RotateCcw, Clock, Coffee, Moon, Edit2, Tag, Settings, Book, Pencil, Calendar } from 'lucide-react'
...
const pastelColors = [
  ...
  "#FFC6FF", "#FFDAB9", "#E6E6FA", "#B0E0E6", "#FFE4E1"
]

export function EnhancedGridLayoutProductivityApp() {
  const [time, setTime] = useState(1500) // 25 minutes in seconds
  const [isRunning, setIsRunning] = useState(false)
  const [timerMode, setTimerMode] = useState('pomodoro')
  const [customTime, setCustomTime] = useState(25)
  const [tasks, setTasks] = useState([
    { id: 1, text: "Terminar teste do excel product design", completed: false, tag: { name: "Design", color: "#BAE1FF" }, dueDate: new Date(2024, 9, 24, 22, 0) },
    { id: 2, text: "Leitura - high performance browser networking", completed: false, tag: { name: "Book", color: "#BAFFC9" }, dueDate: new Date(2024, 9, 25, 13, 0) },
    { id: 3, text: "ler um topico do curso design", completed: false, tag: { name: "Design", color: "#BAE1FF" }, dueDate: new Date(2024, 9, 25, 10, 0) }
  ])
  const [friendTasks, setFriendTasks] = useState([
    { id: 1, text: "Write blog post", completed: true, tag: { name: "Writing", color: "#FFC6FF" }, dueDate: new Date(2024, 9, 25, 15, 0) },
    { id: 2, text: "Prepare presentation", completed: false, tag: { name: "Work", color: "#FFDFBA" }, dueDate: new Date(2024, 9, 26, 9, 0) }
  ])
  const [newTask, setNewTask] = useState("")
  const [editingTask, setEditingTask] = useState(null)
  const [editingTag, setEditingTag] = useState(null)
  const [messages, setMessages] = useState([
    { id: 1, user: "Grazi Masito", text: "How's everyone doing today?", time: "9:56 pm" }
  ])
  ...
  const updateTask = (id, newText, newTag, newDueDate) => {
    setTasks(tasks.map(task => 
      task.id === id ? { ...task, text: newText, tag: newTag, dueDate: newDueDate } : task
    ))
    setEditingTask(null)
    setEditingTag(null)
  }

  const toggleTask = (id, isFriendTask = false) => {
    if (isFriendTask) {
      setFriendTasks(friendTasks.map(task => 
  ...
  const deleteTask = (id, isFriendTask = false) => {
    if (isFriendTask) {
  ...
  const friendCompletedTasksCount = friendTasks.filter(task => task.completed).length
  ...
  return (
    <div className="grid grid-cols-2 gap-6 h-screen bg-gray-50 p-6 w-full max-w-[1600px] mx-auto">
      ...
      <Card className="shadow-md col-span-1">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center text-primary">Team Progress</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              { username: "You", avatarSrc: "https://github.com/shadcn.png", openTasks: tasks.filter(t => !t.completed).length, completedTasks: completedTasksCount },
              { username: "Friend", avatarSrc: "https://api.dicebear.com/6.x/bottts/svg?seed=Felix", openTasks: friendTasks.filter(t => !t.completed).length, completedTasks: friendCompletedTasksCount }
            ].map((user) => (
              <div key={user.username} className="flex flex-col items-center p-4 bg-card rounded-lg shadow-sm">
                <Avatar className="w-12 h-12 mb-2">
                  <AvatarImage src={user.avatarSrc} alt={user.username} />
                  <AvatarFallback>{user.username[0]}</AvatarFallback>
                </Avatar>
                <h3 className="text-base font-medium mb-2">{user.username}</h3>
                <div className="w-full space-y-2">
                  <div className="w-full h-2 bg-secondary-foreground/20 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-primary"
                      style={{ width: `${(user.completedTasks / (user.openTasks + user.completedTasks)) * 100}%` }}
                    />
                  </div>
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>{user.completedTasks} completed</span>
                    <span>{user.openTasks} open</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <CardContent className="p-4">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            <TabsList className="grid w-full grid-cols-2 mb-4">
              ...
              <TabsTrigger value="friendTasks">Friend's Tasks</TabsTrigger>
            </TabsList>
            <TabsContent value="myTasks" className="flex-grow flex flex-col">
              ...
              <ScrollArea className="flex-grow h-[calc(100%-6rem)] pr-4">
                {tasks.map(task => (
                  <div key={task.id} className="flex items-center space-x-2 py-3 border-b border-gray-200">
                    <Checkbox 
                    ...
                    {editingTask === task.id ? (
                      <Input 
                        value={task.text}
                        onChange={(e) => updateTask(task.id, e.target.value, task.tag, task.dueDate)}
                        onBlur={() => setEditingTask(null)}
                        onKeyPress={(e) => e.key === 'Enter' && updateTask(task.id, e.target.value, task.tag, task.dueDate)}
                        className="flex-grow"
                      />
                    ) : (
                                ...
                                <div className="grid gap-2">
                                  <div className="grid grid-cols-3 items-center gap-4">
                                    ...
                                    <Input
                                      id="tag-name"
                                      value={task.tag.name}
                                      onChange={(e) => updateTask(task.id, task.text, { ...task.tag, name: e.target.value }, task.dueDate)}
                                      className="col-span-2"
                                    />
                                  </div>
                                  <div className="grid grid-cols-3 items-center gap-4">
                                    ...
                                    <div className="col-span-2 flex flex-wrap gap-2">
                                      {pastelColors.map((color) => (
                                        <Button
                                          ...
                                          onClick={() => updateTask(task.id, task.text, { ...task.tag, color }, task.dueDate)}
                                        />
                                      ))}
                                    </div>
                                  </div>
                              ...
                              <CalendarComponent
                                ...
                                onSelect={(date) => {
                                  if (date) {
                                    ...
                                    updateTask(task.id, task.text, task.tag, newDate)
                                  }
                                }}
                                initialFocus
                              />
          ...
          <ScrollArea className="flex-grow mb-4 pr-4 h-[calc(100%-6rem)]">
            {messages.map(message => (
              <div key={message.id} className="mb-4">
                ...
                <div className="mt-1 ml-10 p-2 bg-gray-100 rounded-md text-sm">{message.text}</div>
              </div>
            ))}
          </ScrollArea>
